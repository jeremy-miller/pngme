...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        let (header, data) = value.split_at(HEADER_LENGTH);
        if header != Png::STANDARD_HEADER {
            return Err(eyre!("Invalid PNG header"));
        }
        let mut chunks: Vec<Chunk> = Vec::new();
        let mut remaining_data = data;
        while !remaining_data.is_empty() {
            if remaining_data.len() < CHUNK_LENGTH_SIZE + CHUNK_TYPE_SIZE + CHUNK_CRC_SIZE {
                return Err(eyre!("Invalid chunk format"));
            }
            let length: [u8; 4] = remaining_data[0..CHUNK_LENGTH_SIZE]
                .try_into()
...
impl Display for Png {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let s: Vec<String> = self.chunks.iter().map(|c| c.to_string()).collect();
        write!(f, "{}", s.join(""))
    }
}
